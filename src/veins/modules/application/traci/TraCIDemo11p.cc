...
void TraCIDemo11p::onTask(Task* frame)
{
    ...
    if (newTask->getSenderType() == 1 && newTask->getSenderAddress() == myId) {
        ...
        std::string command = "C:\\Users\\29701\\anaconda3\\python.exe vehState.py " + toString(curPosition) + " " + toString(newTask->getDeadlinePos()) + " " + toString(newTask->getSenderPos()) + " " + toString(newTask->getCPU()) + " " + toString(newTask->getMem()) + " " + toString(newTask->getOperationTime()) + " " + newTask->getName();
...
void TraCIDemo11p::handlePositionUpdate(cObject* obj)
{
    ...
    std::string command = "C:\\Users\\29701\\anaconda3\\python.exe posRecord.py " + externalId + " " + roadIdTmp + " " + toString(simTime());
    int result = system(command.c_str());
    for(it = connectedRSUs.begin(); it != connectedRSUs.end(); it++) {
    ...
    // if (mobility->getRoadId() != lastroadID){
        ...
        if (lastroadID.at(0) != ':'){
            ...
            if ((U_Random() > generateP || startflag == 1) && ifSend < maxTask){
                ...
                std::string command = "C:\\Users\\29701\\anaconda3\\python.exe vehDecision.py 123 " + externalId + " " + roadId + " " + RSUinfo + " " + toString(curPosition) + " " + toString(task->getCPU());
