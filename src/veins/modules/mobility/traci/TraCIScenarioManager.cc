...
void TraCIScenarioManager::updateModulePosition(cModule* mod, const Coord& p, const std::string& edge, double speed, Heading heading, VehicleSignalSet signals, std::string objectId)
{
    // update position in TraCIMobility
    auto mobilityModules = getSubmodulesOfType<TraCIMobility>(mod);
    for (auto mm : mobilityModules) {
        mm->nextPosition(p, edge, speed, heading, signals, objectId);
    }
}
...
void TraCIScenarioManager::processVehicleSubscription(std::string objectId, TraCIBuffer& buf)
{
    ...
    for (uint8_t j = 0; j < variableNumber_resp; ++j) {
        ...
        if (isokay != RTYPE_OK) {
            ...
            buf >> speed;
            EV_DEBUG << "new ADD speed value " << speed << endl;
            numRead++;
        }
    ...
    if (!mod) {
        ...
        EV_DEBUG << "hereModule " << typeid( objectId ).name() << endl;
        updateModulePosition(mod, p, edge, speed, heading, VehicleSignalSet(signals), objectId);
    }
}
