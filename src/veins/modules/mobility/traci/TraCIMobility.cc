...
void TraCIMobility::nextPosition(const Coord& position, std::string road_id, double speed, Heading heading, VehicleSignalSet signals, std::string objectId)
{
    ...
    changePosition(objectId, road_id);
    ASSERT(getCurrentDirection() == heading.toCoord() and getCurrentDirection() == getCurrentOrientation());
}

void TraCIMobility::changePosition(std::string objectId, std::string road_id)
{
